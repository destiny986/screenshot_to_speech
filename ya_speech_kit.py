...
from yandex_ai_studio_sdk._speechkit.enums import PCM16, LoudnessNormalization
...
def text_to_voice(text, output_device_id) -> None:
    ...
    tts = sdk.speechkit.text_to_speech(
        ...
        # zhanar_ru strict неплохой женский вариант
        voice="ermil",  # [str]
        # role= ,       # [str]
        speed=1,  # [float]

        # Громкость чтения диктора
    ...
    with OutputStream(
        samplerate=SAMPLERATE, device=output_device_id, channels=1
    ) as out:
        # Basic method - .run_stream, which yields result chunk by chunk
        for partial_result in tts.run_stream(text):
            print(
                f"Chunk [{round(partial_result.start_ms/1000)}, {round(partial_result.end_ms/1000)}] seconds: {partial_result.text}"
            )
...
if __name__ == "__main__":
    ...
    text_to_voice(
        "Плодородные некогда угодья превратились в блёклую пустошь, по которой были разбросаны деревянные остовы заброшенных мельниц",
        output_device_id,
    )
